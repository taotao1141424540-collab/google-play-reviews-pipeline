...
| G5 | 每次都产出**一份人读摘要**，让 mentor/我自己能 30 秒判断这次能不能用 | 试图让监控自动“决定用不用” |
...
## 2. 核心约定（回答之前反复确认的四个问题）

> 这一节直接写成 FAQ 形式，避免歧义。

**Q1：监控会打断采集/清洗/EDA/入库吗？**
**不会。** 现有 01–06 的脚本照跑照输出，监控层在最后独立运行，只读别人写好的 CSV/DB。

**Q2：出现 ERROR 会怎么样？数据会丢吗？**
**不丢。** ERROR 只是一个**体检结果标签**，当前 run 已经生成的 `data/` `reports/` `data/warehouse/` 都保留在磁盘上。ERROR 的作用是：
- 在 `monitoring_report.md` 里用显眼的位置标出这次不健康；
- `check_drift_and_alerts.py` 的退出码为 1，作为“信号灯”留给未来的 CI 使用。

**Q3：退出码 0 / 1 会不会实际阻止什么？**
**MVP 不会。** 你当前手动跑时，0 和 1 都只是信号灯。未来若把监控脚本放在 `load_to_sqlite.py` 之前、用 `&&` 串联，ERROR 才会自动阻断入库（那是 Phase 2 的事）。

**Q4：谁来决定这批数据用不用？**
**人。** 打开 `reports/monitoring/monitoring_report.md` 读头部摘要即可判断。WARN 可用但要留意；ERROR 意味着交付承诺没达成，建议重跑或排查。

---

## 3. 业务场景与需要监控的“红线”

### 3.1 业务承诺（与 mentor 已对齐的要点）
...
> **这些就是 mentor 可以一眼 review 的“质量门清单”。**
...
### 5.1 Run-level logging — “这次跑过没、跑对没”
...
**数据来源**
全部已经在脚本里算过（例如 `len(out_df)` / `len(clean_all)` / `len(reviews)` / per-app `fetched=/failed=`）。MVP 可以不接入这一层，让 `collect_run_metrics.py` 自己合成一条；Phase 2 再让每个脚本自己上报。

**失败行为**
`_runlog.py` 的约定：脚本抛异常时**先记 failed，再 re-raise**。`rows_out=None` 也要写一行。

---

### 5.2 Data quality checks — “数据本身健不健康”

**做什么**
把你已有的 `reports/quality_report.csv` 和 `reports/raw_collection_metrics.csv` **宽表化 → 追加到** `reports/monitoring/data_quality_history.csv`。
...
### 5.3 Distribution tracking — “分布有没有悄悄变样”
...
**约束**
- **不存原始行**、**不存 PNG**——那些还在 EDA 目录里即可。
- 指标缺失（例如某次没跑 C 节）→ 列留空即可，流程继续。

---

### 5.4 Alerts & deviation — “不对劲时要被看到”
...
- **评分分布 PSI**：
  ...
  - 阈值：`psi_rating_max: 0.20`（经验值，可调）。
  - 处理 `p_i=0`：加一个 1e-6 的平滑常数避免 `ln(0)`。
...
- **冷启动**：历史样本 `< baseline_window` 时，仅跑硬阈值，**跳过**漂移计算，并在报告里写“Drift checks skipped: insufficient history (N=2)”。

---

## 7. `config/monitoring.yml` 设计

```yaml
# 硬阈值（越过即 ERROR）
thresholds:
  raw_rows_min: 5000
  apps_count_min_delta: 2         # 允许比预期少 2 个 app
...
drift:
  ...
  en_share_drop_max: 0.10        # 英文占比下降超过 10pp 报 WARN
...
python scripts/07_monitor/check_drift_and_alerts.py   # 失败也只是退出码 1，不影响已生成产物
```

### 8.2 Phase 2（可选，把监控变“守门员”）
...
| `reports/monitoring/monitoring_report.md` | 人读摘要 | **每次覆盖**（本次 run 专属） |
...
1. **Week 1**：落 `collect_run_metrics.py` + `data_quality_history.csv` + `distribution_history.csv`；跑至少 3–5 次建立基线。第一个 `monitoring_report.md` 就可以发给 mentor 看。
2. **Week 2**：接上 `check_drift_and_alerts.py` + `monitoring.yml` + `alerts.csv`；与 mentor 一起敲定硬阈值具体数值（§3.2 已给初稿）。
3. **Week 3（可选）**：接上 `_runlog.py` 给 01/02/05 套一层；开始记录 `pipeline_runs.jsonl`。
4. **Phase 2（mentor 同意后再做）**：把监控脚本前置到入库之前，实现 ERROR 自动阻断入库；或接 GitHub Actions 做邮件/Slack 通知。

---

## 12. 非目标（明确不做）

- 不做服务化监控（不上 Prometheus / Datadog / Grafana）。
- 不在业务脚本里嵌入任何告警判断。
- 不自动重跑失败的 run。
- 不做**实时**监控；本流水线天然是批处理，监控也按 run 粒度。
- 不自动通知（邮件/Slack）——MVP 只产文档。

---

## 13. 附录 A：红线速查表（给 mentor 审阅用）
...
| `clean_en_rows` | `< 8 000` | **违反对 mentor 的 ≥10k 英文交付承诺** |
| `missing_key_fields_rate` | `> 0.5%` | review_id / score / at / content 任一大面积缺失 |
| `sqlite.row_counts(reviews) vs source_rows` | 不等 | 入库丢行/截断 |

---

## 14. 附录 B：对 mentor 的一句话回复（可直接用）

> Hi John, thanks. I’m adding a lightweight monitoring layer under `scripts/07_monitor/` that is strictly read-only w.r.t. the existing pipeline. It writes structured run logs (`logs/pipeline_runs.jsonl`), appends run-level quality/distribution metrics to time-series CSVs, and produces a human-readable `monitoring_report.md` each run. In this MVP the monitor only observes — business scripts still run and produce outputs regardless of alert level; the monitor’s exit code is a signal, not an enforcer. Hard thresholds are tied to our delivery contract (e.g. `clean_en_rows ≥ 8000`, `parseable_time_rate ≥ 0.99`, `duplicate_rate ≤ 0.02`). I’ll share the first report once baselines accumulate, then we can decide together whether to promote it into a quality gate before warehouse load.
